...
async def update_feature_permission(req: Feature, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_feature", RoleAction.EDIT.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
...
async def find_by_role_id(role_id: str, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_feature", RoleAction.VIEW.value, mongo_conn=_crud.mongo):  # 1 untuk izin baca
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
