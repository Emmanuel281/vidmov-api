...
async def create(req: DocType, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsdoctype", RoleAction.ADD.value, mongo_conn=_crud.mongo):  # 2 untuk izin simpan baru
...
async def update_by_id(doctype_id: str, req: DocTypeUpdate, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsdoctype", RoleAction.EDIT.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
            raise PermissionError("Access denied")
...
async def get_all_data(
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsdoctype", RoleAction.VIEW.value, mongo_conn=_crud.mongo):  # 1 untuk izin baca
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
...
async def update_status(doctype_id: str, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsdoctype", RoleAction.DELETE.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
