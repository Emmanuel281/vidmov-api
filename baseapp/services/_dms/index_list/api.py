...
async def create(req: IndexList, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsindexlist", RoleAction.ADD.value, mongo_conn=_crud.mongo):  # 2 untuk izin simpan baru
...
async def update_by_id(index_id: str, req: IndexListUpdate, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsindexlist", RoleAction.EDIT.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
...
async def get_all_data(
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsindexlist", RoleAction.VIEW.value, mongo_conn=_crud.mongo):  # 1 untuk izin baca
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
...
async def update_status(index_id: str, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_dmsindexlist", RoleAction.DELETE.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
