...
async def create(req: Enum, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_enum", RoleAction.ADD.value, mongo_conn=_crud.mongo):  # 2 untuk izin simpan baru
...
async def update_by_id(enum_id: str, req: EnumUpdate, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_enum", RoleAction.EDIT.value, mongo_conn=_crud.mongo):  # 4 untuk izin simpan perubahan
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
...
async def get_all_data(
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_enum", RoleAction.VIEW.value, mongo_conn=_crud.mongo):  # 1 untuk izin baca
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
...
@router.get("/from/{model_from}/{model_name}", response_model=ApiResponse)
async def find_by_id(model_from: str, model_name: str, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        ...
        response = _crud.get_from_model(model_from,model_name)
    return ApiResponse(status=0, message="Data found", data=response)
...
async def delete_by_id(enum_id: str, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    ...
    with CRUD() as _crud:
        if not permission_checker.has_permission(cu.roles, "_enum", RoleAction.DELETE.value, mongo_conn=_crud.mongo):  # 8 untuk izin hapus
            raise PermissionError("Access denied")
        _crud.set_context(
            user_id=cu.id,
            org_id=cu.org_id,
