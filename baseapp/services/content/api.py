...
@cbor_or_json
async def update_by_id(content_id: str, req: Request, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
...
@cbor_or_json
async def update_status(content_id: str, req: Request, cu: CurrentUser = Depends(get_current_user)) -> ApiResponse:
    if not permission_checker.has_permission(cu.roles, "content", 4):  # 4 untuk izin simpan perubahan
        raise PermissionError("Access denied")
